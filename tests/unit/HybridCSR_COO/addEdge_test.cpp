...
TEST_F(HybridStorageShardTest, AddEdge_Basic) {
  ...
  EXPECT_TRUE(graph->impl_addEdge(4, 5, 25).isOK()) << "Failed to add edge (4,5)";
  EXPECT_TRUE(graph->impl_addEdge(1, 5, 35).isOK()) << "Failed to add edge (1,5)";
...
TEST_F(HybridStorageShardTest, AddEdge_NonExistentVertices) {
  ...
  EXPECT_FALSE(graph->impl_addEdge(99, 1, 10).isOK()) << "Added edge with non-existent source";
  EXPECT_FALSE(graph->impl_addEdge(1, 99, 10).isOK()) << "Added edge with non-existent destination";
  EXPECT_FALSE(graph->impl_addEdge(88, 99, 10).isOK()) << "Added edge with both vertices non-existent";
}
...
TEST_F(HybridStorageShardTest, AddEdge_NegativeWeights) {
  ...
  EXPECT_TRUE(graph->impl_addEdge(1, 2, -100).isOK()) << "Failed to add edge with negative weight";
...
TEST_F(HybridStorageShardTest, AddEdge_ZeroWeights) {
  graph->impl_addVertex(1);
  graph->impl_addVertex(2);
  EXPECT_TRUE(graph->impl_addEdge(1, 2, 0).isOK()) << "Failed to add edge with zero weight";
  auto [weight, status] = graph->impl_getEdge(1, 2);
  EXPECT_TRUE(status.isOK()) << "Edge (1,2) not found";
...
TEST_F(HybridStorageShardTest, AddEdge_String) {
  ...
  EXPECT_TRUE(string_graph->impl_addEdge("prasad", "omkar", 1.5).isOK()) << "Failed to add string edge";
  EXPECT_TRUE(string_graph->impl_addEdge("omkar", "tejas", 2.7).isOK()) << "Failed to add string edge";
  EXPECT_TRUE(string_graph->impl_addEdge("prasad", "tejas", 3.14159).isOK()) << "Failed to add string edge";
...
TEST_F(HybridStorageShardTest, AddEdge_SelfLoop) {
  graph->impl_addVertex(1);
  EXPECT_TRUE(graph->impl_addEdge(1, 1, 10).isOK()) << "Failed to add self-loop";
