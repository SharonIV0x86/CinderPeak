...
// Added tests fto validate thread safety for HybridStorage.
TEST_F(HybridCSRCOOTest, ConcurrentReads) {
  for (int i = 0; i < 100; ++i) {
    graph->impl_addVertex(i);
  }
  for (int i = 0; i < 100; ++i) {
...
TEST_F(HybridCSRCOOTest, MixedReadWrite) {
  for (int i = 0; i < 50; ++i)
    graph->impl_addVertex(i);
...
TEST_F(HybridCSRCOOTest, StressTestMultipleThreads) {
...
TEST_F(HybridCSRCOOTest, ProperRaceDetection) {
  for (int i = 0; i < 10; ++i)
    graph->impl_addVertex(i);
...
TEST_F(HybridCSRCOOTest, PerformanceRegression) {
  for (int i = 0; i < 2000; ++i)
    graph->impl_addVertex(i);
