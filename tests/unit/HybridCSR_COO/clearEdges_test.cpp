...
TEST_F(HybridStorageShardTest, ClearEdges_Basic) {
  ...
  EXPECT_TRUE(graph->impl_getEdge(1, 2).second.isOK()) << "Edge (1,2) not found";
  EXPECT_TRUE(graph->impl_getEdge(4, 5).second.isOK()) << "Edge (4,5) not found";
  EXPECT_TRUE(graph->impl_clearEdges().isOK()) << "Failed to clear edges";
  EXPECT_FALSE(graph->impl_getEdge(1, 2).second.isOK()) << "Edge (1,2) still exists";
  EXPECT_FALSE(graph->impl_getEdge(4, 5).second.isOK()) << "Edge (4,5) still exists";
  for (int v : vertices) {
    EXPECT_TRUE(graph->impl_hasVertex(v)) << "Vertex " << v << " removed unexpectedly";
  }
}

// Test clearing edges in an empty graph
TEST_F(HybridStorageShardTest, ClearEdges_EmptyGraph) {
  EXPECT_TRUE(graph->impl_clearEdges().isOK()) << "Failed to clear edges in empty graph";
...
TEST_F(HybridStorageShardTest, ClearEdges_String) {
  ...
  EXPECT_TRUE(string_graph->impl_clearEdges().isOK()) << "Failed to clear string edges";
  EXPECT_FALSE(string_graph->impl_doesEdgeExist("prasad", "omkar")) << "String edge still exists";
  EXPECT_TRUE(string_graph->impl_hasVertex("prasad")) << "String vertex removed unexpectedly";
  EXPECT_TRUE(string_graph->impl_hasVertex("omkar")) << "String vertex removed unexpectedly";
}