...
public:
  ...
  // Graph serialization: save graph to file
  auto save(const std::string &filepath) -> typename Serialization::GraphSerializer<VertexType, EdgeType>::SerializationResult
  {
    ...
    return Serialization::GraphSerializer<VertexType, EdgeType>::serialize(
        vertices, edges, options, filepath);
  }

  // Graph deserialization: load graph from file
  auto load(const std::string &filepath) -> typename Serialization::GraphSerializer<VertexType, EdgeType>::SerializationResult
  {
    ...
    auto result = Serialization::GraphSerializer<VertexType, EdgeType>::deserialize(
        filepath, vertices, edges, options);
    ...
    // Recreate peak_store with loaded options
    PeakStore::GraphInternalMetadata metadata(
        "cinder_graph", Traits::isTypePrimitive<VertexType>(),
        Traits::isTypePrimitive<EdgeType>(),
        Traits::isGraphWeighted<EdgeType>(),
    ...
    peak_store = std::make_unique<PeakStore::PeakStore<VertexType, EdgeType>>(
        metadata, options, PolicyConfiguration());
