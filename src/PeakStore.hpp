  ...
  namespace PeakStore
  {
    ...
    public:
      ...
      std::vector<VertexType> getAllVertices() const
      {
        ...
        const auto &vertex_data = ctx->adjacency_storage->getVertexDataMap();
        vertices.reserve(vertex_data.size());
        for (const auto &pair : vertex_data)
        {
      ...
      std::vector<std::tuple<VertexType, VertexType, EdgeType>> getAllEdges() const
      {
        ...
        const auto &adj = ctx->adjacency_storage->getInternalAdjacency();
        const auto &vertex_data = ctx->adjacency_storage->getVertexDataMap();

        for (const auto &pair : adj)
        {
          VertexId src_id = pair.first;
          auto src_it = vertex_data.find(src_id);
          if (src_it == vertex_data.end())
            continue;

          const VertexType &src = src_it->second;
          for (const auto &edge : pair.second)
          {
            VertexId dest_id = edge.first;
            auto dest_it = vertex_data.find(dest_id);
            if (dest_it == vertex_data.end())
              continue;

            const VertexType &dest = dest_it->second;
            const EdgeType &weight = edge.second;
            edges.push_back(std::make_tuple(src, dest, weight));
          }
        }
        return edges;
      }

      // Get graph creation options
      const GraphCreationOptions &getGraphOptions() const
      {
        return *ctx->create_options;
      }
