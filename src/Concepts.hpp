...
template <typename T>
struct is_primitive_or_string : std::disjunction<std::is_arithmetic<T>, std::is_same<T, std::string>> {};
...
template <typename T>
struct is_comparable_vertex<T, std::void_t<decltype(std::declval<T>() < std::declval<T>())>> : std::true_type {};
...
template <typename T>
struct is_primitive_enum_or_string : std::disjunction<is_primitive_or_string<T>, is_enum_vertex<T>> {};

template <typename T>
constexpr bool is_primitive_enum_or_string_v = is_primitive_enum_or_string<T>::value;

// Weighted numeric edge
template <typename T>
struct is_weighted_numeric_edge : std::integral_constant<bool, is_weighted_v<T> && is_numeric_edge_v<T>> {};
...
struct is_unweighted_integral_vertex : std::integral_constant<bool, is_unweighted_v<E> && std::is_integral<V>::value> {
};

template <typename V, typename E>
constexpr bool is_unweighted_integral_vertex_v = is_unweighted_integral_vertex<V, E>::value;
...
#define STATIC_ASSERT_WEIGHTED(E) \
  static_assert(CinderPeak::Traits::is_weighted_v<E>, "EdgeType must be weighted (not Unweighted).")

#define STATIC_ASSERT_UNWEIGHTED(E) \
  static_assert(CinderPeak::Traits::is_unweighted_v<E>, "EdgeType must be Unweighted.")
