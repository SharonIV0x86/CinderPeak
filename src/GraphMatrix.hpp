...
public:
    ...
    operator[](const VertexType &src) const {
        return EdgeAccessor<VertexType, EdgeType>(
            const_cast<GraphMatrix &>(*this), src);
    }

    friend class EdgeAccessor<VertexType, EdgeType>;

    // Static function to toggle console logging
    static void togglePLogging(bool enable) {
        CinderPeak::PeakStore::PeakStore<VertexType, EdgeType>::togglePLogging(enable);
    }
};
